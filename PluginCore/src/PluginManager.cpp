...
PluginInstance* PluginManager::Load(const std::string &strName,int &errCode)
{
	...
	strNameTemp = strNameTemp + "d";
#endif
	std::map<std::string, DynamicLib *>::iterator iter = m_vecPluginLibs.find(strNameTemp);
	if (iter == m_vecPluginLibs.end())	//不存在就需要插入
	{
		DynamicLib* pLib = new DynamicLib;
		if (pLib != NULL)
		{
			pLib->LoadLib(strNameTemp.c_str());
			m_vecPluginLibs.insert(make_pair(strNameTemp, pLib));
	...
	else if (iter != m_vecPluginLibs.end())		//如果存在，在插件列表里面寻找名字是strNameTemp的插件
	{
		for (int i = 0; i < m_vecPlugins.size(); i ++)
		{
			if (strNameTemp == m_vecPlugins[i]->GetDisplayName())
			{
				errCode = 1;
...
bool PluginManager::UnLoad(const std::string &strName)
{
	...
	for (; iter != m_vecPluginLibs.end(); ++iter )
	{
		...
		if (strcmp(pLib->GetName(),strNameTemp.c_str()) == 0)
		{
